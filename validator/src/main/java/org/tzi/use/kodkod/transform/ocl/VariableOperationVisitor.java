...
public class VariableOperationVisitor extends DefaultExpressionVisitor {
	...
	public void visitAttrOp(ExpAttrOp exp) {
		...
		if (attributeClass == null) {
			...
			if (sourceType instanceof ObjectType) {
				...
				throw new TransformationException(
						"Cannot determine type of source expression for " + StringUtil.inQuotes(exp.toString()) + ".");
			}
		}
		...
		if (attribute != null) {
			...
			// hanhdd-Begin
			// throw new TransformationException("Cannot find attribute " +
			// exp.attr().name() + ".");
			org.tzi.kodkod.model.type.Type sourceType = new TypeConverter(model).convert(exp.objExp().type());
			if (sourceType instanceof ObjectType) {
				attributeClass = ((ObjectType) sourceType).clazz();
			 ...
			 * System.out.println("Hanhdd::exp.objExp().toString()=" +
			 * exp.objExp().toString());
			 * System.out.println("Hanhdd::exp.objExp().type().toString()=" +
			 * exp.objExp().type().toString());
			 * System.out.println("Hanhdd::exp.objExp().type().shortName()=" +
			 * exp.objExp().type().shortName());
	...
	private void handleDerivedAttribute(ExpAttrOp exp, DerivedAttribute attribute) {
		...
		if (collectionType != CollectionType.OBJECT) {
		...
		// FIXME correct?
		if (attribute.derivedExpression().type().isKindOfCollection(VoidHandling.EXCLUDE_VOID)) {
			collectionType = CollectionType.SET; // Default to SET for collections
		} else {
	...
	private void handleRegularAttribute(ExpAttrOp exp, IAttribute attribute) {
		boolean isSequence = attribute.type().isSequence();
		sourceType = exp.type();

		// Determine collection type
		if (isSequence) {
			collectionType = CollectionType.SEQUENCE;
	...
	public void visitNavigation(ExpNavigation exp) {
		...
		if (association != null) {
			...
			throw new TransformationException(
					"Cannot find association " + StringUtil.inQuotes(mAssociation.name()) + ".");
		}
	}

	private void handleDerivedAssociationNavigation(ExpNavigation exp, DerivedAssociation assoc, IAssociationEnd from,
			IAssociationEnd to) {
		// TODO recursion detector
		...
		if (assoc.isBinaryAssociation()) {
			...
			if (to == derEnd) {
				...
				if (nonDerEnd == null) {
					throw new TransformationException(
							"Could not identify non-derived association end of binary association "
				...
				object = ((Expression) oldObject).in(newObject)
						.comprehension(x.oneOf(nonDerEnd.associatedClass().inheritanceOrRegularRelation()));
				collectionType = CollectionType.SET;

				if (to.multiplicity().isObjectTypeEnd()) {
					// if calculated result is empty -> continue to work with Undefined instead of
					// empty relation
				...
				object_type_nav = (collectionType == CollectionType.OBJECT);
				variables.remove("self");
				variables.remove(p.getName());
				if (oldSelf != null) {
					variables.put("self", oldSelf);
	...
	private void handleUnionAssociationNavigation(ExpNavigation exp, UnionAssociation association,
	...
	private void handleRegularAssociationNavigation(ExpNavigation exp, IAssociation association,
			MNavigableElement source,
		...
		if (fromRole == -1 || toRole == -1 || fromRole == toRole) {
			throw new TransformationException(
					"Cannot find correct associationEnd indexes for navigation from " + source.nameAsRolename()
		...
		invokeMethod("navigation", arguments, CollectionType.OBJECT);
	}
	...
	public void visitNavigationClassifierSource(ExpNavigationClassifierSource exp) {
		...
		if (t.isKindOfAssociation(VoidHandling.EXCLUDE_VOID)) {
			...
			invokeMethod("navigationClassifier", arguments, CollectionType.OBJECT);
		} else {
	 ...
	 */
	private int associationClassEnd(MNavigableElement source, IAssociation association, int fromIndex,
			boolean fromRole) {
		if (source.nameAsRolename().toLowerCase().equals(association.name().toLowerCase())) {
			if (fromRole) {
				...
				if (!association.isBinaryAssociation()) {
					...
					collectionType = associationEnd.multiplicity().isObjectTypeEnd() ? CollectionType.OBJECT
							: CollectionType.SET;
				}
